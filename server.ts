...
async function resolveUser(req: AuthRequest) {
  ...
  const resolveTask = (async () => {
    ...
    if (initialRole && user.role !== initialRole) {
      await updateUserRole(user.id, initialRole);
...
// Update / Edit other user's profile and role (Admin only)
app.put('/api/users/:id', authUser, requireRoles('admin'), async (req: ExtendedAuthRequest, res) => {
  try {
    ...
    const { displayName, email, role, avatarUrl } = req.body;
    ...
    const updated = await updateUserProfile(targetUserId, { displayName, email, role, avatarUrl });
    if (!updated) {
...
// Add / Invite team member with assigned role (Admin only)
app.post('/api/users/invite', authUser, requireRoles('admin'), async (req: ExtendedAuthRequest, res) => {
  try {
    ...
    const member = await createTeamMember({ email, displayName, role });
    await logActivity(
      req.appUser!.id,
      req.appUser!.email,
      'INVITE_TEAM_MEMBER',
      'user',
      String(member.id),
      `Added team member ${displayName} (${email}) with role ${role.toUpperCase()}`
    );
